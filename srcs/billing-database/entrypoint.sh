...
create_db_and_user() {
    ...
    runuser -u postgres -- $PG_BIN/psql -d billing_db -c "GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO billing_user;" 2>/dev/null || true
    runuser -u postgres -- $PG_BIN/psql -d billing_db -c "GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO billing_user;" 2>/dev/null || true
    runuser -u postgres -- $PG_BIN/psql -d billing_db -c "GRANT CREATE ON SCHEMA public TO billing_user;" 2>/dev/null || true
...
if [ "$DB_CHECK" == "not_running" ] || [ ! -f "$DATA_DIR/billing_db.initialized" ]; then
    create_db_and_user
    touch "$DATA_DIR/billing_db.initialized"
fi
